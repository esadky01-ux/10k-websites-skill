...
import { synthesizeSpeech, ttsConfigured, TtsError } from "@/server/voice/realtime";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[voice-agent/tts]", err);
    const detail = err instanceof TtsError ? `upstream ${err.status}: ${err.detail}` : (err as Error)?.message ?? String(err);
    return NextResponse.json({ error: "tts-upstream", detail: detail.slice(0, 300) }, { status: 502 });
  }
}
