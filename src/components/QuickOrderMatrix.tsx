...
import { products, formatPackaging } from "@/data/products";
...
export default function QuickOrderMatrix() {
  ...
  const active = categories.some((c) => c.slug === requested) ? requested : "";
  const [query, setQuery] = useState("");
  const searchRef = useRef<HTMLInputElement>(null);

  const selectCategory = (slug: string) => {
    router.replace(slug ? `/siparis?kategori=${slug}` : "/siparis", { scroll: false });
  };
    ...
    <div>
      ...
      <div className="sticky top-[57px] z-30 -mx-4 border-b border-cream-200 bg-cream-50/95 px-4 py-3 backdrop-blur sm:-mx-6 sm:px-6 md:top-[97px] lg:top-[69px]">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 lg:flex-row lg:items-center">
          <div className="flex gap-1.5 overflow-x-auto pb-1 lg:flex-1 lg:pb-0" role="tablist" aria-label="Ürün grubu filtreleri">
            <button
              type="button"
              role="tab"
              ...
              className={`inline-flex shrink-0 items-center gap-1.5 rounded-full px-4 py-2 text-sm font-semibold transition ${
                active === "" ? "bg-ink-900 text-white" : "bg-white text-ink-700 ring-1 ring-cream-200 hover:ring-ink-300"
              }`}
            >
            ...
            {categories.map((c) => (
              <button
                ...
                className={`shrink-0 rounded-full px-4 py-2 text-sm font-semibold transition ${
                  active === c.slug ? "bg-brand-600 text-white" : "bg-white text-ink-700 ring-1 ring-cream-200 hover:ring-ink-300"
                }`}
              >
                {c.name}
              </button>
          ...
          <div className="relative lg:w-80">
            ...
            <input
              ...
              aria-label="Ürün ara"
              className="w-full rounded-full border border-cream-200 bg-white py-2.5 pl-9 pr-9 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20"
            />
            {query && (
              <button
                type="button"
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full p-1 text-ink-500 hover:bg-cream-100"
              >
                <X className="h-4 w-4" />
              </button>
            )}
      ...
      <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-500">
            {activeCat?.description ?? "Koli ve paket/adet sayılarını girin, sepet otomatik güncellenir."}
          </p>
        </div>
        <p className="text-sm text-ink-500">
          <span className="font-semibold text-ink-900">{filtered.length}</span> ürün listeleniyor
        </p>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="mt-8 rounded-2xl border border-dashed border-cream-200 bg-white p-12 text-center">
          <p className="font-semibold text-ink-900">Aramanızla eşleşen ürün bulunamadı</p>
          <p className="mt-1 text-sm text-ink-500">
            Farklı bir kelime deneyin veya WhatsApp üzerinden bize ürünü sorun.
          </p>
        </div>
      ) : (
            ...
            <table className="w-full min-w-[760px] text-left text-sm">
              <thead className="bg-cream-100 text-[11px] font-bold uppercase tracking-wider text-ink-500">
                <tr>
                  ...
                  <th className="px-4 py-3">Ambalaj Formatı</th>
                  <th className="px-4 py-3">Fiyat</th>
                  <th className="px-4 py-3 text-center">Koli</th>
                  <th className="px-4 py-3 text-center">Paket / Adet</th>
                </tr>
              </thead>
                ...
                <tbody key={cat.slug} className="divide-y divide-cream-200">
                  {!active && (
                    <tr className="bg-cream-50">
                      <td colSpan={5} className="px-4 py-2 text-xs font-bold uppercase tracking-wider text-brand-600">
                        {cat.name}
                      </td>
                    </tr>
                  )}
                  {items.map((p) => {
                    ...
                    const inCart = !!line;
                    return (
                      <tr key={p.id} className={`transition ${inCart ? "bg-brand-50/50" : "hover:bg-cream-50"}`}>
                        ...
                        <td className="whitespace-nowrap px-4 py-3">
                          ...
                          <span className="block text-xs text-ink-500">
                            Koli = {p.unitsPerCase} {p.unitLabel.toLowerCase()}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <a
                            ...
                            className="inline-flex items-center gap-1.5 rounded-full border border-cream-200 px-3 py-1.5 text-xs font-semibold text-ink-700 hover:border-ink-300"
                          >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-center">
                            <QtyCounter
                              label="Koli"
                              value={line?.cases ?? 0}
                              onChange={(v) => cart.setQuantity(p.id, v, line?.units ?? 0)}
                            />
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex justify-center">
                            <QtyCounter
                              label={p.unitLabel}
                              value={line?.units ?? 0}
                              onChange={(v) => cart.setQuantity(p.id, line?.cases ?? 0, v)}
                            />
                          </div>
                        </td>
                      </tr>
          ...
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="text-sm">
                <p className="font-bold text-ink-900">{cart.lineCount} kalem sepette</p>
                <p className="text-xs text-ink-500">
            ...
            <button
              type="button"
              onClick={cart.open}
              className="inline-flex items-center gap-2 rounded-full bg-wa px-5 py-2.5 text-sm font-bold text-white hover:bg-wa-dark"
            >
              Sepeti Gör & WhatsApp&apos;a Gönder
