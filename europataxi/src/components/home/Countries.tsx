...
export function Countries({ locale, dict }: CountriesProps) {
  return (
    <section aria-labelledby="countries-title" className="bg-ink-soft py-16 md:py-24">
      <div className="container">
        ...
        <ul role="list" className="mt-12 grid gap-px overflow-hidden rounded-lg border border-line bg-line md:grid-cols-2 xl:grid-cols-4">
          {countries.map((code) => {
            ...
            return (
              <li key={code} className="flex min-w-0 flex-col bg-ink-soft p-6">
                ...
                <ul role="list" className="mt-2 flex flex-wrap gap-2">
                  {cities.map((city) => (
                    <li key={city.id} className="rounded-md border border-line px-2.5 py-1 text-sm text-paper">
                ...
                <ul role="list" className="mt-2 space-y-1.5">
                  {airports.map((airport) => (
                    <li key={airport.id} className="flex items-start gap-2 text-sm text-paper">
                      <Plane aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-taxi" />
                ...
                <Link
                  ...
                  className="mt-auto inline-flex items-center gap-1.5 self-start rounded-sm pt-6 text-sm font-bold text-taxi hover:text-taxi-dark"
                >
