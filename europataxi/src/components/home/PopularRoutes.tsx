...
export function PopularRoutes({ routes, dict }: PopularRoutesProps) {
  const baseId = useId();
  return (
    <section aria-labelledby="popular-routes-title" className="bg-surface py-16 md:py-24">
            ...
            return (
              <li key={route.id}>
                <button
                  ...
                  className="group flex h-full w-full flex-col rounded-lg border border-line bg-surface-2 p-5 text-left transition-colors motion-reduce:transition-none hover:border-taxi-ink focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-taxi-ink focus-visible:ring-offset-2 focus-visible:ring-offset-surface"
                >
                  <span className="flex min-w-0 flex-col gap-1 text-base font-bold text-content">
                    <span className="break-words">{route.fromName}</span>
                    <span className="flex items-start gap-2">
                      <ArrowRight
                        aria-hidden="true"
                        className="mt-1 h-4 w-4 shrink-0 text-taxi-ink motion-safe:transition-transform motion-safe:group-hover:translate-x-0.5"
                      />
                  ...
                  <span id={metaId} className="mt-4 flex w-full items-end justify-between gap-3 border-t border-line pt-4">
                    <span className="flex flex-col">
                      ...
                      <span className="text-lg font-extrabold tabular-nums text-content">{route.price}</span>
